...
public class BankingStructure 
{
    ...
    public static void main( String[] args )
    {
        ...
        if (currentuser!=null) {
         ...
         else{login=false; System.out.println("Username or password is incorrect.");} }
        ...
        if (register) {
        	...
        	String registername="";
        	System.out.println("Welcome customer. Please input your desired username. Must be between 5 and 15 characters long "
        	...
        	do{
        		...
        		if(scanner.hasNext(pattern)) {
        			...
        			scanner.nextLine();
        			System.out.println("Valid names start with a capital letter and only contain alphabet characters.");
        		}
        		if (check) {
        			do {
        				check=false;
        ...
        if(iscustomer) {
        	...
        	check=false;
        	do {
        		System.out.println("Please enter the number for the desired action.\n1. Access/View Accounts\n2. View Pending Applications\n3. Apply for Account\n4. Logout");
        		do {
        				...
        				case "1":
        					if(accountempty) {
        						...
        						do {
        	        				...
        	        				System.out.println("Please enter the number next to the account you want to access.");
        	        				if(scanner.hasNextInt()) {
        	        						...
        	        						do {
        	        							...
        	        							if(scanner.hasNextInt()) {
        	        								...
        	        								case 4:
        	        									...
        	        									System.out.println(bank.returnaccountInfo(accessaccountid)+"\n");
        	        									break;
        	        								case 5:
        	        									check=true;
        	        									break;
        						...
        						do {
        							...
        							if(scanner.hasNextInt()) {        
        								...
        								case "1":
        									...
        									System.out.println("Applied! Your application accountid number is: " + currentcustomer.getPending().get(currentcustomer.getPending().size()-1) + ".") ;
        									check=true;
        									break;
        								case "2": // applying to existing account.
        							        ...
        							        do{
        							        	...
        							        	if (currentcustomer.applyjoint(accountid, accountList, applicationList)) {
        							        		System.out.println("Applied for joint account to account " + accountid);
        							        		check=true;
        							        	}
        							        	else {
        							        		check=true;
        							        		System.out.println("Error. Account with that accountid does not exist. Or you are attempting to reapply to the same account. Or you are attempting to apply to your own account.\n");
        							        	}
        							        }else {
        							        	scanner.nextLine();
        							        	System.out.println("Enter a valid accountid. Id's are between 10000 and 99999.");
        							        }
        							        }while(!check);
        									break;
        								case "3":
        							        ...
        							        do{
        							        	...
        							        	if(scanner.hasNext(pattern)) {
        							        	accountid= Integer.parseInt(scanner.nextLine());
        							        	if (currentcustomer.applicationjoint(accountid, accountList, applicationList)) {
        							        		System.out.println("Applied for joint account to application " + accountid);
        							        		check=true;
        							        	}
        							        	else {
        							        		System.out.println("Error. Application with that accountid does not exist. Or you are attempting to reapply to the same application. Or you are attempting to apply to your own application.\n");
        							        		check=true;
        							        	}
        							        }else {
        							        	scanner.nextLine();
        			...
        			if(scanner.hasNextInt()) {        
    						...
    						do {
								...
								if (scanner.hasNextInt()) {
									...
									if (temp > 0 && temp <= (bank.customerList.size())) {
										System.out.println(bank.customerList.get(temp - 1).customerInfo(bank.accountList));
        				...
        				case "2": 
        					...
        					if(bank.applicationList.isEmpty()) {
    	        				...
    	        				System.out.println("Please enter the number next to the application you want to approve or deny.");
    	        						...
    	        						System.out.println("1. Approve\n2. Deny");
    	        							do {
    	        								if(scanner.hasNextInt()) {
    	        									switch(Integer.parseInt(scanner.nextLine())){
    	        									case 1:
    	        										currentemployee.approve(bank.applicationList.get(temp-1).getAccountid(), applicationList, customerList, accountList);
    	        										System.out.println("Application approved.");
    	        										approvecheck=true;
    	        										break;
    	        									case 2:
    	        										currentemployee.deny(bank.applicationList.get(temp-1).getAccountid(), applicationList, customerList, accountList);
        			...
        			if(scanner.hasNextInt()) {        
        				...
        				case "1":
    						...
    						do {
								System.out.println("Current customers:");
								System.out.println(bank.viewCustomers());
								System.out.println("Please enter the number next to the customer you want to access.");
								if (scanner.hasNextInt()) {
									int temp = Integer.parseInt(scanner.nextLine());
									...
									if (temp > 0 && temp <= (bank.customerList.size())) {//This is where we want accountinfo, accountstuff
										...
										do {System.out.println("Please enter the number for the desired action.\n1. View customer information\n2. Edit Accounts \n3. Back");
											if(scanner.hasNextInt()) {
												customercheck=false;
												switch(Integer.parseInt(scanner.nextLine())) {
												case 1:
													System.out.println(currentcustomer.customerInfo(bank.accountList));
													break;
												case 2:
													if(currentcustomer.getAccountList().size()==0) {
						        						...
						        						do {
						        	        				...
						        	        				if(scanner.hasNextInt()) {
						        	        					...
						        	        					int accessaccountid;
						        	        					if (temp2>0 && temp2 <= (currentcustomer.getAccountList().size())) {
						        	        						check=false;
						        	        						accessaccountid=currentcustomer.getAccountList().get(temp2-1);   	        						
						        	        						do {
						        	        							System.out.println("Account "+ accessaccountid +":\nPlease enter the number for the desired action.\n1. Deposit\n2. Withdraw\n3. Transfer\n4. All Account Information\n5. Delete Account\n6. Back");
						        	        							if(scanner.hasNextInt()) {
						        	        								switch(Integer.parseInt(scanner.nextLine())) {
						        	        								...
						        	        								case 5:
						        	        									...
						        	        									do {
						        	        										Pattern pattern = Pattern.compile("(?=.{1}$)[yYnN]");
						        	        										if(scanner.hasNext(pattern)) {
						        	        											switch(scanner.nextLine()) {
						        	        												case "y":
						        	        												case "Y":
						        	        													currentadmin.delete(accessaccountid, bank.customerList, bank.accountList);
						        	        													deletecheck=true;
						        	        													check=true;
						        	        													break;
						        	        												case "n":
    	        						...
    	        						System.out.println("1. Approve\n2. Deny");
    	        							do {
    	        								if(scanner.hasNextInt()) {
    	        									switch(Integer.parseInt(scanner.nextLine())){
    	        									case 1:
    	        										currentadmin.approve(bank.applicationList.get(temp-1).getAccountid(), applicationList, customerList, accountList);
    	        										System.out.println("Application approved.");
    	        										approvecheck=true;
    	        										break;
    	        									case 2:
    	        										currentadmin.deny(bank.applicationList.get(temp-1).getAccountid(), applicationList, customerList, accountList);
    	        										System.out.println("Application denied.");
    	        										approvecheck=true;
    	        										break;
    	        									default:
