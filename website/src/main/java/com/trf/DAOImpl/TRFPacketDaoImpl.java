...
public class TRFPacketDaoImpl implements TRFPacketDao{
	...
	public ArrayList<TRFPacket> getTRFPacketsbyID(int employeeID){
		...
		try {
			PreparedStatement prest = conn.prepareStatement("SELECT TRF_ID, EVENT_DATETIME, EVENT_LOCATION, EVENT_DESCRIPTION, GRADING_FORMAT, PASSING_GRADE, EVENT_TYPE_ID, WORK_RELATED_JUSTIFICATION, SUPERVISOR_APPROVAL_EXIST, HEAD_APPROVAL_EXIST, BC_APPROVAL, WORK_TIME_MISSED, PROJECTED_REIMBURSEMENT FROM TUITION_REIMBURSEMENT_FORM WHERE USER_ID=? AND DENIED='N' ORDER BY TRF_ID");
